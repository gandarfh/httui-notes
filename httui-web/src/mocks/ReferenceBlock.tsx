  ...
  return (
    <Flex direction="column" gap={0}>
      ...
      <ExecutableBlockShell
        ...
        inputSlot={
          <Flex align="center" gap={2} px={3} py={2}>
            <Badge size="sm" colorPalette="blue" variant="solid" fontFamily="mono" fontSize="xs">
              {httpMethod}
            </Badge>
            <Text fontFamily="mono" fontSize="xs" color="fg.muted">{httpUrl}</Text>
          </Flex>
        }
        outputSlot={
          <Box>
      ...
      <ExecutableBlockShell
        ...
        inputSlot={
          <Box>
            <Flex align="center" gap={2} px={3} py={2} borderBottom="1px solid" borderColor="border">
              ...
              <Text fontFamily="mono" fontSize="xs" color="fg.muted">{dbConnection}</Text>
            </Flex>
            <Box as="pre" fontFamily="mono" fontSize="xs" color="purple.300" p={3} m={0} lineHeight="1.6" whiteSpace="pre-wrap">
              {dbQuery.split(referenceHighlight).map((part, i, arr) =>
                i < arr.length - 1 ? (
                  <Text as="span" key={i}>
                    {part}
                    <Text as="span" bg="brand.400/15" color="brand.400" px={1} rounded="sm">
                      {referenceHighlight}
                    </Text>
                  </Text>
                ) : (
        ...
        outputSlot={
          <Box overflowX="auto">
            <Box as="table" w="100%" css={{ borderCollapse: "collapse" }} fontSize="xs" fontFamily="mono">
              <Box as="thead">
                <Box as="tr">
                  {dbColumns.map((col) => (
                    <Box as="th" key={col.name} textAlign="left" px={3} py={1.5} color="fg.muted" fontWeight="600" borderBottom="1px solid" borderColor="border" bg="bg.subtle">
                      {col.name}
                    </Box>
                  ))}
                </Box>
              ...
              <Box as="tbody">
                {dbRows.map((row, i) => (
                  <Box as="tr" key={i}>
                    {dbColumns.map((col) => (
                      <Box as="td" key={col.name} px={3} py={1.5} color="fg" borderBottom="1px solid" borderColor="border">
                        {String(row[col.name])}
                      </Box>
                    ))}
                  </Box>
