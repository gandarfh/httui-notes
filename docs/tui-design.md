...
**Foco atual (2026-04-26):** paridade do bloco DB com o desktop. Auditoria de gap consolidada na seção "Gap de paridade" no fim desta seção.
...
### Status entregue (sessão 2026-04-26)

P0+P1 completos, parte do P2 entregue:
- ✅ 04.1 Bind params (segurança) · 04.2 Multi-statement · 04.3 Schema cache · 04.4a Completion engine · 04.4b Schema autocomplete (com bare-column scope-aware)
- ✅ 04.5 Token `timeout=` · 04.6 Cache hash validation · 04.7 Refs autocomplete `{{...}}` · 04.8 Errors line/col + visual · 04.9 Read-only + confirm dialog
- ✅ 05.1 Result tabs (Result/Messages/Plan/Stats, cycle via `gt`/`gT` em InBlockResult) · 05.2 EXPLAIN integration (ex command `:explain`)
- ⏳ Pendente P2: Story 08 (display modes), Story 11 (edição inline fence), Story 05.3 (export menu CSV/MD/INSERT)
- ⏳ Refactor parcial: 8 helpers DB-domain movidos pra `commands::db`; restante (`apply_run_block` família) ainda em `vim::dispatch`. Detalhes em `docs/backlog/21-tui-block-widgets.md` "Próxima sessão".

Stats: 408 testes passando (+112 desde início), 0 warnings clippy em `httui-tui/`. ~14 commits coerentes em `main`.

### Gap de paridade Desktop × TUI (auditoria 2026-04-26)
...
- Schema panel UI lateral — desktop STUB.
- Run history persistence + viewer.
- Resolved bindings debug panel.

**Em paridade (✅✅):** connection picker, virtualized table, cell rendering null/JSON/long, row detail (TUI superior — vim motions full + Y JSON copy), pagination (TUI auto-prefetch vs desktop button), cancel via `Ctrl+C`, single-call execution, refs cross-block.
