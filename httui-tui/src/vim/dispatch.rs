...
use crate::commands::db::{
    compute_db_cache_hash, db_summary_from_value, is_cacheable_query,
    is_unscoped_destructive, is_writing_query, resolve_connection_id_sync,
    save_db_cache_async, strip_leading_sql_comments,
};
...
// `compute_db_cache_hash` moved to `crate::commands::db`.

// Detectors moved to `crate::commands::db`. The imports at the top
// of this file bring `is_cacheable_query`, `is_writing_query`,
// `is_unscoped_destructive`, and `strip_leading_sql_comments` into
// scope so existing call sites keep compiling without a deeper
// refactor of `apply_run_block` itself (next session).

// `db_summary_from_value`, `save_db_cache_async`, and
// `resolve_connection_id_sync` moved to `crate::commands::db`.
// Imports at the top bring them into local scope so the existing
// `apply_run_block` / `handle_db_block_result` / `rebuild_completion_popup`
// call sites keep working until they're moved too (next session).
...
/// Bridge for `crate::commands::db::run_explain` — the public
/// entry point for running an arbitrary SQL string against the
/// block at `segment_idx` while keeping the block's own
/// `params["query"]` text untouched. Lives here only because
/// `run_db_block_inner` is still private to dispatch; once that
/// migrates to `commands::db` this wrapper goes away.
pub fn run_db_block_inner_for_explain(
    app: &mut App,
    segment_idx: usize,
    wrapped: String,
) {
